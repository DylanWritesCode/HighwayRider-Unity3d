...
public class MainMenu : MonoBehaviour {
	...
	void Start ()
	{
		...
		// Read and save native screen resolution in game first scene
		...
		if (PlayerPrefs.GetInt ("Effect") == 3)
			GameObject.FindObjectOfType<MobileColorGrading> ().enabled = true;
		else
			GameObject.FindObjectOfType<MobileColorGrading> ().enabled = false;
		...
		if (PlayerPrefs.GetInt ("FirstRun") != 3) 
		{
			...
			PlayerPrefs.SetFloat ("SteerSpeed", 3.7f);
			// Open first bike
